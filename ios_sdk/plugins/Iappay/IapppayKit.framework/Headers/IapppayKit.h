 ...
 * @params:tokenValue   如果使用OpenID登录的就将登录信息传入支付SDK,否则传nil.
 * @params:payDelegate  支付结果处理对象
 **/
- (NSInteger)makePayForTrandInfo:(NSString *)trandInfo
                openIDTokenValue:(NSString *)tokenValue
               payResultDelegate:(id<IapppayKitPayRetDelegate>)payDelegate;
